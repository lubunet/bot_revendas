...
def limpar_fluxo(user_id):
    ...
    limpar_trava_admin(user_id)


def callback_admin_protegido(callback_data):
    protegidos = {
        "listar_usuarios_painel",
        "listar_usuarios_todos",
        "listar_usuarios_meus",
        "painel_revendas",
        "add_revenda",
        "del_revenda",
        "toggle_uuid_expired_mode",
        "toggle_uuid_mode",
        "criar_usuario",
        "criar_teste",
        "criar_usuario_auto",
        ...
        "deletar_usuario",
        "apagar_expirados",
        "confirmar_deletar_expirados",
        "alt_data_revenda",
        "alt_limite_revenda",
        "suspender_revenda",
        "renovar_revenda",
        "usuarios_online_menu",
        "usuarios_online_meus",
        "usuarios_online_todos",
        "relatorio_revendas",
        "relatorio_individual_revenda",
        "relatorio_subrevendas_revendas",
        "alt_nome_usuario_revenda",
        "consultar_usuario_individual",
        "mais_funcoes_admin",
        "criar_backup_admin",
        "confirmar_restore_backup",
        "cancelar_restore_backup",
        "teste_velocidade_admin",
        "calcular_limite_revenda",
        "toggle_auto_backup"
    }

    if callback_data in protegidos:
        return True

    return isinstance(callback_data, str) and callback_data.startswith("admin_rel_subs:")


def admin_usuario_esta_ocupado(user_id):
    agora = time.time()
    expira_em = ADMIN_USERS_BUSY.get(user_id, 0)

    if expira_em > agora:
        return True

    if user_id in ADMIN_USERS_BUSY:
        ADMIN_USERS_BUSY.pop(user_id, None)

    return False


def iniciar_trava_admin(user_id, segundos=ADMIN_BUSY_TIMEOUT):
    ADMIN_USERS_BUSY[user_id] = time.time() + segundos


def limpar_trava_admin(user_id):
    ADMIN_USERS_BUSY.pop(user_id, None)
...
def menu(message):
    ...
    if tg and usuario_eh_revenda(message):
        revenda.resetar_fluxo_revenda(message.from_user.id)
    ...
    if tg and usuario_eh_subrevenda(message):
        sub.resetar_fluxo_sub(message.from_user.id)
...
def callback(c):
    ...
    if callback_admin_protegido(c.data):
        if admin_usuario_esta_ocupado(c.from_user.id):
            bot.answer_callback_query(c.id, "Aguarde...", show_alert=True)
            return

        iniciar_trava_admin(c.from_user.id)
    # -----------------------------------------------------
    # LISTAR USUÁRIOS
    # -----------------------------------------------------
    if c.data == "listar_usuarios_painel":
        ...
        limpar_trava_admin(c.from_user.id)
        return
...
def montar_texto_relatorio_individual_revenda(telegram_user):
    ...
    subs = listar_subrevendas_da_revenda(telegram_user)
    limite_gasto_subs = calcular_limite_reservado_subrevendas(telegram_user) if subs else 0
    ...
    if subs:
        linhas.append(f"<b>Limite gasto com subs:</b> {esc(limite_gasto_subs)}")
...
def gerar_pdf_relatorio_individual_revenda(telegram_user):
    ...
    def desenhar_topo():
        ...
        y -= 16
        if subs:
            c.drawString(margem_lateral, y, f"Limite gasto com subs: {limite_gasto_subs}")
            y -= 16
...
def telegram_revenda_valido(telegram_user):
    ...
    return bool(re.fullmatch(r"@[A-Za-z0-9_]{1,32}", tg))
...
def receber_novo_nome_usuario_revenda(message):
    ...
    telegram_novo = message.text.strip()
    ...
    telegram_novo = normalizar_telegram_user(telegram_novo)
...
def obter_dados_relatorio_revenda(telegram_user):
    ...
    usado = limite_total - limite_restante
    if usado < 0:
        usado = 0
...
def texto_eh_comando_menu(texto):
    return bool(re.fullmatch(r"/menu(?:@[A-Za-z0-9_]+)?", str(texto or "").strip(), re.IGNORECASE))


def tratar_menu_durante_fluxo(message):
    if not texto_eh_comando_menu(getattr(message, "text", "")):
        return False
...
def erro_telegram_message_nao_modificada(error):
    return "message is not modified" in str(error or "").lower()


def erro_telegram_transiente(error):
    return isinstance(
        error,
        (
...
def criar_wrapper_retry_api_telegram(metodo, tentativas=2, espera=1.2, ignorar_nao_modificado=False):
    def wrapper(*args, **kwargs):
        for tentativa in range(1, tentativas + 1):
            try:
                return metodo(*args, **kwargs)
            except Exception as e:
                if ignorar_nao_modificado and erro_telegram_message_nao_modificada(e):
                    return None

                if not erro_telegram_transiente(e) or tentativa >= tentativas:
                    raise

                print(
                    f"[TELEGRAM RETRY] metodo={getattr(metodo, '__name__', 'api')} "
                    f"tentativa={tentativa + 1}/{tentativas} erro={repr(e)}"
                )
                time.sleep(espera)
...
def configurar_wrappers_telegram():
    original_register_next_step_handler = bot.register_next_step_handler

    def register_next_step_handler_com_menu(message, callback, *args, **kwargs):
        def callback_wrapper(step_message):
            if tratar_menu_durante_fluxo(step_message):
                return
            return callback(step_message, *args, **kwargs)

        return original_register_next_step_handler(message, callback_wrapper)

    bot.register_next_step_handler = register_next_step_handler_com_menu
    bot.edit_message_text = criar_wrapper_retry_api_telegram(
        bot.edit_message_text,
        tentativas=2,
        espera=0.8,
        ignorar_nao_modificado=True
    )
    bot.edit_message_reply_markup = criar_wrapper_retry_api_telegram(
        bot.edit_message_reply_markup,
        tentativas=2,
        espera=0.8,
        ignorar_nao_modificado=True
...
try:
    ...
    while True:
        ...
        except (requests.exceptions.ReadTimeout, requests.exceptions.ConnectionError, requests.exceptions.Timeout, TimeoutError) as e:
            print(f"[POLLING TRANSIENTE] {repr(e)}")
            time.sleep(3)
        except Exception as e:
